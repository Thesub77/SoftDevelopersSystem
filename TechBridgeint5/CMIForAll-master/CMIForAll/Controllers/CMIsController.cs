...
namespace CMIForAll.Controllers
{
    public class CMIsController : Controller
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            var metaDescripcion = db.Metas.Where(m => m.Id == id).Select(m => m.Descripcion).FirstOrDefault();
            ViewBag.MetaDescripcion = metaDescripcion;

            string query = "SELECT TOP (1000) [Id], [Nombre] FROM [dbo].[Perspectivas]";
            List<Perspectiva> perspectivas = db.Perspectivas.ToList();
            ViewBag.Perspectivas = perspectivas;
